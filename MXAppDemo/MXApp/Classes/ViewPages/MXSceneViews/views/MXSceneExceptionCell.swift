...
class MXSceneExceptionCell: UITableViewCell {
    ...
    public func setupViews() {
        ...
        self.nameLab.pin.right(of: self.iconView).marginLeft(20).right(110).height(21).top(29)
        
        self.bgView.addSubview(self.valueLab)
        self.valueLab.pin.below(of: self.nameLab, aligned: .left).marginTop(4).right(110).height(18)
        
        self.bgView.addSubview(self.actionBtn)
        self.actionBtn.pin.right(20).width(80).height(40).vCenter()
    ...
    public func refreshView(info: MXSceneInfo) {
        ...
        if let value = info.des, value.count > 0 {
            ...
            info.actions.forEach({ (item:MXSceneTACItem) in
                if let obj = item.params as? MXDeviceInfo, let objName = obj.name, let property_list = obj.properties, obj.status != 3 {
